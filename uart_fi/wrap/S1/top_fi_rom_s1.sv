`timescale 1ns/1ps
// ROM-fed FI shell for S1: on-chip stimulus ROM -> DUT; UART only for FI commands.
// External ports kept small for synth/impl. Not a paper fair-kernel top.
module top_fi_rom_s1 (
    input  wire clk,
    input  wire rst,
    input  wire uart_rx,
    ...
    // stimulus image: generate/supply it yourself, see uart_fi/README.md
    localparam INIT_FILE = "S1_input.mem";
    ...
    fi_stim_source #(
        .DEPTH(256), .ADDR_W(8), .DATA_W(280), .BEATS_PER_FRAME(256), .INIT_FILE(INIT_FILE)
    ) u_stim (
    ...
    fi_cmd_path #(.CLKS_PER_BIT(16)) u_cmd (
